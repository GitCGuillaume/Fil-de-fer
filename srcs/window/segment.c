...
/*   Updated: 2021/06/14 23:47:35 by gchopin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (thread->lines[i][j_one])
		{
			...
			if (thread->lines[i][j_one])
			{
				...
				*/
				if (start == 0)
				{
					...
					else if (end_hor->altitude < 0) //&& start_hor->altitude != end_hor->altitude)
						start_hor->y = end_hor->y - (tan(radian_hor) / (end_hor->x - start_hor->x)) - start_hor->altitude;
					else
						start_hor->y = end_hor->y - (tan(radian_hor) / (end_hor->x - start_hor->x));// - start_hor->altitude;
				}
			...
			if (thread->lines[i + 1] && thread->lines[i + 1][j_two])
			{
				...
				/*else if (end_ver.altitude < 0)
					end_ver.y = end_ver.y + (tan(radian_ver) * (end_ver.x - start_ver.x)) - end_ver.altitude;
				else
					end_ver.y = end_ver.y + (tan(radian_ver) * (end_ver.x - start_ver.x));// - end_hor->altitude;
				*/
				//bresenham(thread, start_ver, end_ver);
